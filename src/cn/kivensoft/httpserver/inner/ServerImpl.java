...
import cn.kivensoft.httpserver.Filter;
import cn.kivensoft.httpserver.Headers;
import cn.kivensoft.httpserver.HttpContext;
...
class ServerImpl implements TimeSource {
	...
	private volatile long ticks; /*
									 * number of clock ticks since server
									 * started
									 */
	...
	class Exchange implements Runnable {
			...
			try {
				if (context != null) {
					...
					if (https) {
						...
						sslStreams = new SSLStreams(ServerImpl.this, sslContext,
								chan);
						...
						rawin = new BufferedInputStream(
								new Request.ReadStream(ServerImpl.this, chan));
				...
				if (newconnection) {
					connection.setParameters(rawin, rawout, chan, engine,
							sslStreams, sslContext, protocol, ctx, rawin);
				}
				/*
		...
		void reject(int code, String requestStr, String message) {
			...
			sendReply(code, true,
					"<h1>" + code + Code.msg(code) + "</h1>" + message);
		}
