...
import cn.kivensoft.httpserver.HttpsConfigurator;
import cn.kivensoft.httpserver.HttpsParameters;
...
class SSLStreams {
	...
	SSLStreams(ServerImpl server, SSLContext sslctx, SocketChannel chan)
			throws IOException {
		this.server = server;
		this.time = (TimeSource) server;
		this.sslctx = sslctx;
		this.chan = chan;
		InetSocketAddress addr = (InetSocketAddress) chan.socket()
	...
	private void configureEngine(HttpsConfigurator cfg,
	...
	/**
	 * request the engine to repeat the handshake on this session the handshake
	 * must be driven by reads/writes on the streams Normally, not necessary to
	 * call this.
	 */
	...
	/*
	 * reallocates the buffer by :- 1. creating a new buffer double the size of
	 * the old one 2. putting the contents of the old buffer into the new one 3.
	 * set xx_buf_size to the new size if it was smaller than new size
	 *
	 ...
	 * channel read/writes. It handles the UNDER/OVERFLOW status codes It does
	 * not handle the handshaking status codes, or the CLOSED status code though
	 * once the engine is closed, any attempt to read/write to it will get an
	 * exception. The overall result is returned. It functions
	...
	class EngineWrapper {
		...
		int u_remaining; // the number of bytes left in unwrap_src after an
							// unwrap()
		...
		/*
		 * try to wrap and send the data in src. Handles OVERFLOW. Might block
		 * if there is an outbound blockage or if another thread is calling
		 * wrap(). Also, might not send any data if an unwrap is needed.
		 */
		WrapperResult wrapAndSend(ByteBuffer src) throws IOException {
			return wrapAndSendX(src, false);
		}

		WrapperResult wrapAndSendX(ByteBuffer src, boolean ignoreClose)
				throws IOException {
		...
		/*
		 * block until a complete message is available and return it in dst,
		 * together with the Result. dst may have been re-allocated so caller
		 * should check the returned value in Result If handshaking is in
		 * progress then, possibly no data is returned
		 */
				...
				do {
					...
					if (status == Status.BUFFER_UNDERFLOW) {
						if (unwrap_src.limit() == unwrap_src.capacity()) {
							...
							unwrap_src = realloc(unwrap_src, false,
									BufType.PACKET);
						} else {
							/*
							 * Buffer not full, just need to read more data off
							 * the channel. Reset pointers for reading off
							 * SocketChannel
							 */
	...
	/**
	 * send the data in the given ByteBuffer. If a handshake is needed then this
	 * is handled within this method. When this call returns, all of the given
	 * user data has been sent and any handshake has been completed. Caller
	 * should check if engine has been closed.
	 */
	...
	/**
	 * read data thru the engine into the given ByteBuffer. If the given buffer
	 * was not large enough, a new one is allocated and returned. This call
	 * handles handshaking automatically. Caller should check if engine has been
	 * closed.
	 */
	 ...
	 * being idempotent. eg. if wrapAndSend() is called with no data to send
	...
	/**
	 * represents an SSL input stream. Multiple https requests can be sent over
	 * one stream. closing this stream causes an SSL close input.
	 */
	class InputStream extends java.io.InputStream {
		...
		/**
		 * close the SSL connection. All data must have been consumed before
		 * this is called. Otherwise an exception will be thrown. [Note. May
		 * need to revisit this. not quite the normal close() symantics
		 */
	 ...
	 * is encrypted by the stream. Multiple HTTPS responses can be sent on one
	 * stream. closing this stream initiates an SSL closure
	 */
